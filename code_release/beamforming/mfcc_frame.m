function [ CC, FBE, jacob ] = mfcc_frame( X, fs, alpha, R, M, N, L )
...
%
%           jacob is the Jacobian matrix of CC w.r.t speech
%
%   Example
...
%   Author: Kamil Wojcicki, September 2011
    ...
    K = size(X, 1);              % length of the unique part of the FFT
    nfft = 2 * (K - 1);          % length of FFT analysis 
    ...
    preemph_filter = fft([1 -alpha]', nfft);
    preemph_filter = preemph_filter(1 : K);
    X_preemph = bsxfun(@times, X, preemph_filter);
    ...
    if nargout == 3
        num_frames = size(X, 2);
        jacob = cell(num_frames, 1);
        for t = 1 : num_frames
            jacob_CC_over_MAG = H' * diag(FBE(:, t) .^ -1) * DCT' * diag(lifter);
            jacob{t} = conj(diag(preemph_filter)) * diag(X_preemph(:, t) ./ MAG(:, t))...
                * jacob_CC_over_MAG;
        end
    end
